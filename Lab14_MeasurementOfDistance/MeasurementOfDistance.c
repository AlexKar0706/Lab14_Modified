...
// once the final solution with interrupts and mailbox are operational,
// you should use this main to build the solution for the real board with SSD1306 OLED display
int main4(void){
	TExaS_Init(ADC0_AIN1_PIN_PE2, SSI0_Real_Nokia5110_Scope);
	// initialize ADC0, channel 1, sequencer 3
	SSD1306_Init(SSD1306_WITH_PLL); // initialize SSD1306 OLED display
	// initialize SysTick for 40 Hz interrupts
	// initialize profiling on PF1 (optional)
	EnableInterrupts();
	SSD1306_SetCursor(7,0);// print a welcome message  (optional)
	SSD1306_Write_Str("Lab 14");
	while(1){
		// read mailbox
		
		// output to SSD1306 OLED
		SSD1306_SetCursor(5, 3);
		SSD1306_Write_Str(String);
	}
}
